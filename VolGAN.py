...
def SPXData(datapath, surfacespath):
    ...
    sfts_df = pd.read_csv(surfacespath, header = None)
    surfaces_transform = sfts_df.iloc[:, 1:].values
    prices = np.array(price_data['close'])
    prices_prev = np.array(price_data['prev_close'])
    
    tau = np.array([1/12, 1/6, 1/4, 1/3, 1/2, 3/4, 1.0, 1.5, 2])
    m = np.array([0.7, 0.8, 0.9, 0.95, 1.0, 1.05, 1.1, 1.2, 1.3])
    taus, ms = np.meshgrid(tau,m)
    dates_dt = pd.to_datetime(np.array(price_data['date']))

    log_rtn = np.array(price_data['log_return'])
...
def DataPreprocesssing(datapath, surfacepath):
    ...
    print("log_rtn_tm1 shape:", log_rtn_tm1.shape)
    print("log_iv_tm1 shape:", log_iv_tm1.shape)
    print("file paths/datapath:", datapath)
    print("file exists:", os.path.exists(datapath))
...
def GradientMatching(gen,gen_opt,disc,disc_opt,criterion,condition_train,true_train,m,tau,ms,taus,n_grad,lrg,lrd,batch_size,noise_dim,device, lk = 9, lt = 9):
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(n_batches):
            ...
            penalties_m = torch.matmul(m_seq,(torch.matmul(matrix_m,fake_surface.T)**2))
            penalties_t = torch.matmul(tsq,(torch.matmul(matrix_t,fake_surface.T)**2))
...
def GradientMatchingPlot(gen,gen_opt,disc,disc_opt,criterion,condition_train,true_train,m,tau,ms,taus,n_grad,lrg,lrd,batch_size,noise_dim,device, lk = 9, lt = 9):
    """
    ...
    #m_seq
    m_seq = torch.zeros((moneyness_t.shape[0] - 1),dtype=torch.float,device=device)
    for i in range(moneyness_t.shape[0]-1):
        m_seq[i] = 1 / ((moneyness_t[i+1] - moneyness_t[i]) ** 2)
    m_seq = m_seq.repeat(lt).unsqueeze(0)

    # t_seq
    t_seq = torch.zeros(((lt - 1) * lk),dtype=torch.float,device=device)
    for i in range(tau_t.shape[0] - 1):
        t_seq[i * lk: (i + 1) * lk] = 1 / ((tau_t[i+1] - tau_t[i]) ** 2)

    # matrix_m
    col = 0
    matrix_m = torch.zeros(((lk - 1) * lt, lk * lt), device=device, dtype=torch.float)
    for i in range((lk - 1) * lt): 
        if (i + 1) % lk == 0:
            col += 1
        matrix_m[i, i + col] = -1
        matrix_m[i, i + 1 + col] = 1

    # matrix_t
    matrix_t = torch.zeros(((lt - 1) * lk, Ngrid), device = device, dtype = torch.float)
    for i in range((lt - 1) * lk):
        matrix_t[i,i] = -1
        matrix_t[i,i+lk] = 1
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(n_batches):
            ...
            penalties_m = torch.matmul(m_seq,(torch.matmul(matrix_m,fake_surface.T)**2))
            penalties_t = torch.matmul(t_seq,(torch.matmul(matrix_t,fake_surface.T)**2))
            m_penalty = penalties_m.sum() / curr_batch_size
            t_penalty = penalties_t.sum() / curr_batch_size
            m_penalty.backward(retain_graph=True)
            total_norm = 0
            for p in gen.parameters():
                param_norm = p.grad.data.norm(2)
    ...
    plt.savefig(f"{task_dir}/BCE_norm_{job_id}.png")
    plt.close()
    ...
    plt.savefig(f"{task_dir}/Smoothness_m_{job_id}.png")
    plt.close()
    ...
    plt.savefig(f"{task_dir}/Smoothness_tau_{job_id}.png")
    plt.close()
    
    print("alpha :", alpha, "beta :", beta)
    return gen,gen_opt,disc,disc_opt,criterion, alpha, beta

def TrainLoopNoVal(alpha,beta,gen,gen_opt,disc,disc_opt,criterion,condition_train,true_train,m,tau,ms,taus,n_epochs,lrg,lrd,batch_size,noise_dim,device, lk = 9, lt = 9):
    """
...
def VolGAN(datapath,surfacepath, tr, noise_dim = 16, hidden_dim = 8, n_epochs = 1000,n_grad = 100, lrg = 0.0001, lrd = 0.0001, batch_size = 100, device = 'cpu'):
    ...
    true_train, true_test, condition_train, condition_test,  m_in,sigma_in, m_out, sigma_out, dates_t,  m, tau, ms, taus = DataTrainTest(datapath=datapath,surfacepath=surfacepath, tr=tr, device=device)
    ...
    gen,gen_opt,disc,disc_opt,criterion, alpha, beta = GradientMatchingPlot(gen,gen_opt,disc,disc_opt,criterion,condition_train,true_train,m,tau,ms,taus,n_grad,lrg,lrd,batch_size,noise_dim,device)
